...
class StoreUserRequest extends FormRequest
{
    ...
    {
        return [
            'name' => ['bail|required|string|min:3|max:30|regex:/^[\p{L}0-9_\s]{3,30}$/u|unique:users', Rule::unique('users')->ignore($this->user)],
            'email' => ['bail|required|string|email:rfc,dns|max:320|unique:users', Rule::unique('users')->ignore($this->user)],
